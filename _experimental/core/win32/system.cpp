...
bool CALL HGE_Impl::System_Start()
{
	...
	for(;;)
	{
		...
		if(bActive || bDontSuspend)
		{
			...
			if(dt >= nFixedDelta + nAdaptiveDelta)
			{
				...
				//PROFILE_BEGIN(HGE_procFrameFunc);
				if(procFrameFunc()) break;
				//PROFILE_END();

				//PROFILE_BEGIN(HGE_procRenderFunc);
				if(procRenderFunc) procRenderFunc();
				//PROFILE_END();

                if (bUseAdaptiveDelta)
                {
                    if (!m_systemBallancer)
                    {
                        m_systemBallancer = new HGE_SystemBallancer();
                    }
                    nAdaptiveDelta = m_systemBallancer->Ballance(timeGetTime() - t0, nFixedDelta);
                }
			...
			// for win-tab hack
			if (m_forceFullScreenTime > 0 && m_forceFullScreenTime <= Timer_GetTime())
			{
				ToggleFullScreenMode();
				m_forceFullScreenTime = -1.0f;
			}
		}
	...
	switch(state)
	{
		case HGE_WINDOWED:		if(VertArray || hwndParent) break;
								if(pD3DDevice && bWindowed != value)
								{
                                    ...
                                    SetWindowLong(hwnd, GWL_STYLE, bWindowed ? styleW : styleFS);
									_GfxRestore();
									_AdjustWindow();
								}
		...
		case HGE_TEXTURECLAMP:
			...
			break;
        case HGE_ADAPTIVE_DELTA:
            bUseAdaptiveDelta = value;
            if (!value)
            {
                nAdaptiveDelta = 0;
            }
...
void CALL HGE_Impl::System_SetStateFunc(hgeFuncState state, hgeCallback value)
{
	switch(state)
	{
        ...
        case HGE_FULLSCREENFUNC: procFullScreenFunc=value; break;
        case HGE_SHOUDFULLSCREENFUNC: allowFullScreenFunc=value; break;
	}
}
								...
								if(pD3DDevice)
								{
									if((nHGEFPS>=0 && value <0) || (nHGEFPS<0 && value>=0))
									{
										if (value == HGEFPS_VSYNC)
										{
											...
											d3dppW.FullScreen_PresentationInterval = D3DPRESENT_INTERVAL_ONE;
										...
										else
										{
											...
											d3dppW.FullScreen_PresentationInterval = D3DPRESENT_INTERVAL_IMMEDIATE;
...
const char* CALL HGE_Impl::System_GetStateString(hgeStringState state) {
	switch(state) {
		...
		case HGE_APPPATH:		if (szAppPath[0]) return szAppPath; 
								else return 0;
	}
...
HGE_Impl::HGE_Impl()
{
	...
	procExitFunc=0;
    procFullScreenFunc=0;
    allowFullScreenFunc=0;
	szIcon=0;
	...
	m_keyboardHook = NULL;
	m_restoreFullScreenOnFocusGain = false;
	m_forceFullScreenTime = -1.0f;

    m_systemBallancer = NULL;
...
bool HGE_Impl::ToggleFullScreenMode()
{
	pHGE->m_restoreFullScreenOnFocusGain = false;
	pHGE->m_forceFullScreenTime = -1.0f;
	if (pHGE->allowFullScreenFunc && !pHGE->allowFullScreenFunc())
	{
		return false;
	}
	
	pHGE->System_SetState(HGE_WINDOWED, !pHGE->System_GetState(HGE_WINDOWED));
	if (pHGE->procFullScreenFunc)
	{
		pHGE->procFullScreenFunc();
	}
	return true;
}
...
*/
		case WM_ACTIVATE:
			...
			if(pHGE->pD3D && pHGE->bActive != bActivating) pHGE->_FocusChange(bActivating);
			if (bActivating && pHGE->m_restoreFullScreenOnFocusGain && pHGE->System_GetState(HGE_WINDOWED))
			{
				//pHGE->ToggleFullScreenMode();
				//pHGE->m_restoreFullScreenOnFocusGain = false;
				pHGE->m_forceFullScreenTime = pHGE->Timer_GetTime() + 1;
			}
			else if (!bActivating &&
					 pHGE->m_restoreFullScreenOnFocusGain &&
					 pHGE->System_GetState(HGE_WINDOWED) &&
				     pHGE->m_forceFullScreenTime > 0.0f)
			{
				ShowWindow(pHGE->System_GetState(HGE_HWND), SW_MINIMIZE);
				pHGE->m_forceFullScreenTime = -1.0f;
			}
			else
			{
		...
		case WM_SYSKEYDOWN:
			...
			{
				return pHGE->ToggleFullScreenMode();
			}
			else
			{
...
bool SendAeroActivateCommand()
{
	bool left = GetAsyncKeyState(VK_LWIN);
	if (left || GetAsyncKeyState(VK_RWIN))
	{
		INPUT inputEvent;
		ZeroMemory(&inputEvent, sizeof(INPUT));

		inputEvent.type = INPUT_KEYBOARD;

		inputEvent.ki.wVk = left ? VK_LWIN : VK_RWIN;
		SendInput(1, &inputEvent, sizeof(INPUT));

		inputEvent.ki.wVk = VK_TAB;
		SendInput(1, &inputEvent, sizeof(INPUT));
		...
		switch (wParam)
		{
			case WM_KEYDOWN:
			{
				...
				switch(p->vkCode)
				{
					case VK_TAB:
					{
						if (((GetAsyncKeyState(VK_LWIN) & 0x8000) || (GetAsyncKeyState(VK_RWIN) & 0x8000)) && IsCurrentOSVista())
						{
							bool windowed = pHGE->System_GetState(HGE_WINDOWED);
							if (!windowed)
							{
								if (pHGE->ToggleFullScreenMode())
								{
									if (SendAeroActivateCommand())
									{
										pHGE->m_restoreFullScreenOnFocusGain = true;
									}
								}
