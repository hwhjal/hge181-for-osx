...
public:
	...
	bool				CALL	ToggleFullScreenMode();
	...
	bool				(*procExitFunc)();
    bool				(*procFullScreenFunc)();
    bool				(*allowFullScreenFunc)();
	const char*			szIcon;
	char				szWinTitle[256];
	int					nScreenWidth;
	int					nScreenHeight;
	...
	int					CurPrimType;
	int					CurBlendMode;
	HTEXTURE			CurTexture0;
	HTEXTURE			CurTexture1;
	...
	DWORD				t0, t0fps, dt;
	int					cfps;
    bool                bUseAdaptiveDelta;
    int                 nAdaptiveDelta;

    // Stacking run loops
    int                 m_runLoopDepth;
	HHOOK				m_keyboardHook;
	bool				m_restoreFullScreenOnFocusGain;
	float				m_forceFullScreenTime;

    HGE_SystemBallancer *m_systemBallancer;
