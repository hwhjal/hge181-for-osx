...
{
	_init(szFont, NULL, 0, NULL, bMipmap);
}

hgeFont::hgeFont(const char *szFont, void *data, DWORD size, HTEXTURE texture)
{
    _init(szFont, data, size, texture);
}
void hgeFont::_init(const char *szFont, void *data, DWORD size, HTEXTURE texture, bool bMipmap)
{
    char	*desc, *pdesc;
    char	linebuf[256];
    ...
    {
        if(!strncmp(linebuf, FNTBITMAPTAG, sizeof(FNTBITMAPTAG)-1 ) && !hTexture)
        {
            strcpy(buf,szFont);
            pbuf=strrchr(buf,'\\');
