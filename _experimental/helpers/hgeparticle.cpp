    ...
    : opacity(1.0f)
{
	...
	if(!psi) return;
#if __LP64__
	memcpy((char *)&info + 4, psi, sizeof(hgeParticleSystemInfo) - 4);
#else
	memcpy((char *)&info, psi, sizeof(hgeParticleSystemInfo));
#endif
...
void hgeParticleSystem::Render()
{
	...
	for(i=0; i<nParticlesAlive; i++)
	{
        ...
        {
            DWORD color = SETA(par->colColor.GetHWColor(), par->colColor.a * opacity * 255.f);
...
void hgeParticleSystem::Render(float opacity)
{
	int i;
	DWORD col;
	hgeParticle *par=particles;
	...
	{
		if(info.colColorStart.r < 0)
			info.sprite->SetColor(SETA(info.sprite->GetColor(),par->colColor.a*255.0f * opacity));
		else
		{
			hgeColorRGB color(par->colColor);
			color.a *= opacity;
			info.sprite->SetColor(color.GetHWColor());
		}
		info.sprite->RenderEx(par->vecLocation.x*fScale+fTx, par->vecLocation.y*fScale+fTy, par->fSpin*par->fAge, par->fSize*fScale);
		par++;
	}
