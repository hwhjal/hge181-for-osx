...
bool RenderFunc()
{
	...
	// Calculate display time_
	
	hrs=(int)floorf(time_);
	tmp=(time_-hrs)*60.0f;
	mins=(int)floorf(tmp);
	...
	hge->Gfx_BeginScene();
	RenderSimulation();	
	fnt->printf(7, 7, HGETEXT_LEFT, "Keys 1-9 to adjust simulation speed, 0 - real time_\nFPS: %d", hge->Timer_GetFPS());
...
void UpdateSimulation()
{
	...
	if(speed==0.0f) time_=GetTime();
	else
	{
		time_+=hge->Timer_GetDelta()*speed;
		if(time_>=24.0f) time_-=24.0f;
	}
	
	seq_id=(int)(time_/3);
	seq_residue=time_/3-seq_id;
	zenith=-(time_/12.0f*M_PI-M_PI_2);
	...
	if(seq_id>=6 || seq_id<2)
		for(int i=0; i<NUM_STARS; i++)
		{
			...
			if(seq_id>=6) a*=sinf((time_-18.0f)/6.0f*M_PI_2);
			else a*=sinf((1.0f-time_/6.0f)*M_PI_2);
			starA[i]=a;
		}
	...
	a=(cosf(time_/6.0f*M_PI)+1.0f)/2.0f;
	...
	sunS=1.0f-0.3f*sinf((time_-6.0f)/12.0f*M_PI);
	...
	moonS=1.0f-0.3f*sinf((time_+6.0f)/12.0f*M_PI);
	moonGlowS=a*0.4f+0.5f;
	
	// Calculate sea glow
	
	if(time_>19.0f || time_<4.5f) // moon
	{
		a=0.2f; // intensity
		if(time_>19.0f && time_<20.0f) a*=(time_-19.0f);
		else if(time_>3.5f && time_<4.5f) a*=1.0f-(time_-3.5f);
	...
	else if(time_>6.5f && time_<19.0f) // sun
	{
		a=0.3f; // intensity
		if(time_<7.5f) a*=(time_-6.5f);
		else if(time_>18.0f) a*=1.0f-(time_-18.0f);
	...
	if(time_>19.0f || time_<5.0f) // moon
	{
		a=0.12f; // intensity
		if(time_>19.0f && time_<20.0f) a*=(time_-19.0f);
		else if(time_>4.0f && time_<5.0f) a*=1.0f-(time_-4.0f);
		posX=moonX;
	}
	else if(time_>7.0f && time_<17.0f) // sun
	{
		a=0.14f; // intensity
		if(time_<8.0f) a*=(time_-7.0f);
		else if(time_>16.0f) a*=1.0f-(time_-16.0f);
		posX=sunX;
	}
	else a=0.0f;
