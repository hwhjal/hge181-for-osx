...
void hgeSprite::SetMask(HTEXTURE texture, float x, float y, float w, float h)
{
	float texx1, texy1, texx2, texy2;

	hge=hgeCreate(HGE_VERSION);

	float mask_tx=x, mask_ty=y;
	float mask_width=w, mask_height=h;
	float mask_tex_width, mask_tex_height;

	if(texture)
	{
		mask_tex_width = (float)hge->Texture_GetWidth(texture);
		mask_tex_height = (float)hge->Texture_GetHeight(texture);
	}
	else
	{
		mask_tex_width = 1.0f;
		mask_tex_height = 1.0f;
	}
	...
	quad.tex_mask=texture;

	texx1=x/mask_tex_width;
	texy1=y/mask_tex_height;
	texx2=(x+w)/mask_tex_width;
	texy2=(y+h)/mask_tex_height;

	quad.v[0].tx_mask = texx1; quad.v[0].ty_mask = texy1;
	quad.v[1].tx_mask = texx2; quad.v[1].ty_mask = texy1;
	quad.v[2].tx_mask = texx2; quad.v[2].ty_mask = texy2;
	quad.v[3].tx_mask = texx1; quad.v[3].ty_mask = texy2;

	quad.blend_mask=BLEND_DEFAULT;
}

void hgeSprite::SetMaskOffset(float dx, float dy)
{
	mask_dx = dx;
	mask_dy = dy;
}

void hgeSprite::GetMaskOffset(float* dx, float* dy) const
{
	*dx = mask_dx;
	*dy = mask_dy;
}
...
void hgeSprite::Render(float x, float y)
{
	...
	if (quad.tex_mask)
	{
		int mtw = hge->Texture_GetWidth(quad.tex_mask);
		int mth = hge->Texture_GetHeight(quad.tex_mask);
		quad.x_mask_offset = (mask_dx / quad.x_mask_scale - mask_hotX) / mtw;
		quad.y_mask_offset = (mask_dy / quad.y_mask_scale - mask_hotY) / mth;
		//quad.x_mask_offset = ((mask_dx + tx) / quad.x_mask_scale - mask_hotX) / mtw;
		//quad.y_mask_offset = ((mask_dy + ty) / quad.y_mask_scale - mask_hotY) / mth;
	}

	hge->Gfx_RenderQuad(&quad);
}

void hgeSprite::RenderWithOpacity(float x, float y, float opacity)
{
	DWORD color = this->GetColor();
	DWORD rcolor = SETA(color, static_cast<float>(GETA(color)) * opacity);
	this->SetColor(rcolor);
...
void hgeSprite::RenderEx(float x, float y, float rot, float hscale, float vscale)
{
	...
	if (quad.tex_mask)
	{
		quad.x_mask_offset = (mask_dx / quad.x_mask_scale - mask_hotX) / hge->Texture_GetWidth(quad.tex_mask);
		quad.y_mask_offset = (mask_dy / quad.y_mask_scale - mask_hotY) / hge->Texture_GetHeight(quad.tex_mask);

		//quad.x_mask_offset *= hscale;
		//quad.y_mask_offset *= vscale;

		if (rot != 0.f)
		{
			float xmot = quad.x_mask_offset;
			float ymot = quad.y_mask_offset;
			quad.x_mask_offset = xmot*cost - ymot*sint;
			quad.y_mask_offset = xmot*sint + ymot*cost;
		}
	}
...
void hgeSprite::RenderStretch(float x1, float y1, float x2, float y2)
{
	...
	if (quad.tex_mask)
	{
		float scale_x = hge->Texture_GetWidth(quad.tex) * (quad.v[1].tx - quad.v[0].tx) / (x2 - x1);
		float scale_y = hge->Texture_GetHeight(quad.tex) * (quad.v[2].ty - quad.v[1].ty) / (y2 - y1);
		quad.x_mask_offset = (mask_dx - mask_hotX / scale_x) / hge->Texture_GetWidth(quad.tex_mask) * scale_x;
		quad.y_mask_offset = (mask_dy - mask_hotY / scale_y) / hge->Texture_GetHeight(quad.tex_mask) * scale_y;
	}

	hge->Gfx_RenderQuad(&quad);
}

void hgeSprite::RenderStretchWithOpacity(float x1, float y1, float x2, float y2, float opacity)
{
	DWORD color = this->GetColor();
	DWORD rcolor = SETA(color, static_cast<float>(GETA(color)) * opacity);
	this->SetColor(rcolor);

	this->RenderStretch(x1, y1, x2, y2);
...
void hgeSprite::Render4V(float x0, float y0, float x1, float y1, float x2, float y2, float x3, float y3)
{
	...
	if (quad.tex_mask)
	{
		float scale_x = hge->Texture_GetWidth(quad.tex) * (quad.v[1].tx - quad.v[0].tx) / (x3 - x1);
		float scale_y = hge->Texture_GetHeight(quad.tex) * (quad.v[2].ty - quad.v[1].ty) / (y3 - y1);
		quad.x_mask_offset = (mask_dx - mask_hotX / scale_x) / hge->Texture_GetWidth(quad.tex_mask) * scale_x;
		quad.y_mask_offset = (mask_dy - mask_hotY / scale_y) / hge->Texture_GetHeight(quad.tex_mask) * scale_y;

		quad.x_mask_offset = (mask_dx - mask_hotX) / hge->Texture_GetWidth(quad.tex_mask);
		quad.y_mask_offset = (mask_dy - mask_hotY) / hge->Texture_GetHeight(quad.tex_mask);
	}
...
bool hgeSprite::TestSpritePoint(float x, float y, int alphaThreshold) const
{
    bool result = false;
	...
	float tx, ty, tw, th;
	GetCurrentTextureRect(tx, ty, tw, th);
		...
		x < tw && y < th)
	{
        ...
        unsigned int* pixels = (unsigned int*)
			hge->Texture_Lock(texture, true, (int)x, (int)y, 1, 1);
		if (pixels)
		{
			
			unsigned int color = pixels[0];
			if (!hge->System_GetState(HGE_BYTEORDER)) color;
				else color = pixels[3];
			if (!hge->System_GetState(HGE_BYTEORDER) && alphaThreshold <= GETA(color)) result = true;
				else if (hge->System_GetState(HGE_BYTEORDER) && alphaThreshold <= GETB(color)) result = true;					
		}
