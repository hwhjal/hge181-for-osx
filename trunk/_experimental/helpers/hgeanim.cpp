...
hgeAnimation::hgeAnimation(const hgeSprite &sprite)
    : hgeSprite(sprite)
{
    this->orig_width      = hge->Texture_GetWidth(sprite.GetTexture(), true);
	this->bPlaying        = false; 
	this->fSpeed          = 1.0f; 
	this->fSinceLastFrame = -1.0f;
	this->Mode            = HGEANIM_FWD | HGEANIM_LOOP;
	this->nDelta          = 1;
	this->nFrames         = 1;
	this->nCurFrame       = 0;
...
void hgeAnimation::Rewind(bool forward)
{
    SetFrame(forward ? nFrames - 1 : 0);
}
